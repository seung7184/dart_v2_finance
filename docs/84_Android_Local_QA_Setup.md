...
- If Expo Router reports `Attempted to navigate before mounting the Root Layout component`, inspect `apps/mobile/app/_layout.tsx`. The root layout must render the root `Stack` or `Slot` navigator on the first render. Auth redirects should run from a child gate after mount, using `useRouter`, `useSegments`, and an effect instead of returning `<Redirect>` before the root navigator exists.
